...
int main(int argc, char **argv)
{
    ...
    std::vector<std::tuple<std::string, std::string, unsigned int>> modes = {
            {"GPU naive", "matrix_multiplication_naive", 1},
            {"GPU local memory", "matrix_multiplication_local_memory", 1},
            {"GPU MWPT", "matrix_multiplication_MWPT", 8}
    };

    for (const auto& mode : modes) {
        std::string print_name, kernel_name;
        unsigned int work_per_thread;
        std::tie(print_name, kernel_name, work_per_thread) = mode;
...

        {
            ...
            for (int iter = 0; iter < benchmarkingIters; ++iter) {
                ...
                matrix_multiplication_kernel.exec(
                        gpu::WorkSize(work_group_size, work_group_size / work_per_thread, global_work_size_x, global_work_size_y / work_per_thread),
                        as_gpu,bs_gpu, cs_gpu, M, K, N);

                t.nextLap();
            }
            std::cout << print_name << ": " << t.lapAvg() << "+-" << t.lapStd() << " s" << std::endl;
            std::cout << print_name << ": " << gflops / t.lapAvg() << " GFlops" << std::endl;
        }
